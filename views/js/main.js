...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    // Moved neseccary variable out of the for loop for optimization
    console.log("size:" + size);
      var randomPizzas = document.getElementsByClassName("randomPizzaContainer");
      var dx = determineDx(randomPizzas[0], size);
      var newwidth = (document[0].offsetWidth + dx) + 'px';
...
// Moved pizza div out of for loop for optimization
...
// According to the width of the calculation of each line there are several pizza
function pizzasInRow(width) {
  if (width >= 1200) {
    return 8;
...
// Calculate the total number of pizza
function calPizzaNum(height, cols){
  // console.log(cols * (parseInt(height / s)));
  return cols * (parseInt(height / s) + 1);
}
...
document.addEventListener('DOMContentLoaded', function() {
  // Add pizza based on browser window size
  var window_Width = window.innerWidth;
  var window_Height = window.innerHeight;
  var cols = pizzasInRow(window_Width);
  s = 256;
  var pizzas_Num = calPizzaNum(window_Height, cols);
  var moveSelector = document.getElementById("movingPizzas1");
  var elem;
  for (var i = 0; i < pizzas_Num; i++) {
